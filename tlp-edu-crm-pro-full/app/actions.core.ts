      ...
      if (en?.id) {
        ...
        if (presentCount > 0 && presentCount % 8 === 0) {
          ...
          if (outstanding > 0) {
            ...
            if (parent?.email) {
              const html = `<p>Hi ${parent.name || 'Parent'},</p><p>This is a friendly reminder that fees are due for ${student?.first_name || 'your child'} after ${presentCount} sessions.</p><p>Outstanding balance: <strong>$${outstanding.toFixed(2)}</strong>.</p><p>Thank you,<br/>The Learners Pathway</p>`
              await sendEmail(parent.email, `Fee Reminder for ${student?.first_name || 'Student'}`, html)
            }
          }
        }
      }
